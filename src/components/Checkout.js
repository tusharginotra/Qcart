...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      //setNewAddress({isAddingNewAddress:true});
      let response = await axios.post(`${config.endpoint}/user/addresses`,{"address":newAddress},{
        headers:{ Authorization : `Bearer ${token}`} 
      });
      setAddresses({
        all: response.data,
        selected: response.data.length
    ...
    if (validateRequest(items, addresses)) {
      try {
        ...
        if (e.response) {
          ...
          enqueueSnackbar(
            "Could perform checkout operation. Check that the backend is running, reachable and returns valid JSON.",
            {
              variant: "error",
            }
          );
              ...
              <Box >
                
                {
                  addresses.all.map( (address) => {
                    return (
                    <Box className={addresses.selected === address._id
                      ? "address-item selected"
                      : "address-item not-selected"
                      } key = {address._id} mt={2} onClick={() =>
                      setAddresses({ ...addresses, selected: address._id })
                    }
                      >
                        <Typography my="0.5rem" className="address">
                          {address.address}
                        </Typography>
 
                      <IconButton  onClick={()=>deleteAddress(token,address._id)}>
                        <Delete  />
                        DELETE
                      </IconButton>
                    </Box>
